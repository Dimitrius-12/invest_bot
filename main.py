...
async def bot_msg(message: types.Message):
    match message.text:
        ...
        case "🆘 Тех. поддержка":
            await bot.send_message(message.chat.id, "Для связи с тех.поддержкой нажмите кнопку ниже:", reply_markup=nav.support_kb())
        case '📖 Как начать инвестировать':
            img = InputFile('assets/photo_2023-04-16_18-45-49.jpg')
            await bot.send_photo(message.chat.id, photo=img, caption="В данной статье Вы узнаете обо всех аспектах по работе с нашей платформой", reply_markup=nav.how_to_kb())
        case 'ℹ️ О проекте':
